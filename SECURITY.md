...
live-server serves the directory you point it at, on `127.0.0.1` by default. With `host = "0.0.0.0"` every file under that root is reachable by anyone who can reach the port; `token` gates the live-reload endpoints and the paths `protected_paths` names, and every other file stays readable. The README's Design notes say so. A report that shows a way around the path containment, the token gate or the loopback bind is in scope.
